...
(function (global) {
  ...
  /** Realtime: any app_kv change for session:* keys */
  async function ensureRealtime() {
    ...
    try {
      const sb = await ensureClient();
      realtimeChannel = sb
        .channel('truckit_sessions')
        .on(
          'postgres_changes',
          { event: '*', schema: 'public', table: KV_TABLE },
          function (payload) {
            const row = payload.new || payload.old;
            if (!row || !row.key || String(row.key).indexOf(SESSION_PREFIX) !== 0) return;
            const ref = String(row.key).slice(SESSION_PREFIX.length);
