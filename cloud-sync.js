...
(function (global) {
  ...
  const KEYS = {
    ...
    authProfiles: 'truckit_auth_profiles',
    offices: 'truckit_offices',
    rateRequests: 'truckit_rate_requests'
  };
  ...
  function mergeById(remoteArr, localArr) {
    ...
    if (!rem.length && loc.length) return loc.slice();
    if (!loc.length && rem.length) return rem.slice();
    const map = {};
    function stamp(item) {
      return Date.parse((item && (item.updatedAt || item.createdAt)) || 0) || 0;
    }
    rem.concat(loc).forEach(function (item) {
      if (!item || typeof item !== 'object') return;
      const id = item.id || String(item.email || '').toLowerCase() || JSON.stringify(item);
      const prev = map[id];
      if (!prev) map[id] = item;
      else map[id] = stamp(item) >= stamp(prev) ? Object.assign({}, prev, item) : Object.assign({}, item, prev);
    });
  ...
  async function pullKey(storageKey) {
    ...
    const local = parseMaybe(localStorage.getItem(storageKey));
    const remote = (data && data.value != null) ? (typeof data.value === 'string' ? parseMaybe(data.value) : data.value) : null;

    if (Array.isArray(remote) || Array.isArray(local)) {
      const rem = Array.isArray(remote) ? remote : [];
      const loc = Array.isArray(local) ? local : [];
      if (rem.length === 0 && loc.length > 0) {
        try { await pushKey(storageKey, loc); } catch (e) {}
        return loc;
      }
      const merged = mergeById(rem, loc);
      localStorage.setItem(storageKey, JSON.stringify(merged));
      if (loc.length && rem.length && merged.length !== rem.length) {
        try { await pushKey(storageKey, merged); } catch (e) {}
      }
      return merged;
    }
    ...
    if (remote != null) {
      ...
      try { return JSON.parse(str); } catch (e) { return remote; }
    }
    return local;
  }
  ...
  /** Create auth user without replacing the current session (throwaway client). */
  async function signUpIsolated(email, password, metadata) {
    const cfg = getConfig();
    if (!cfg) throw new Error('Supabase not configured');
    if (typeof global.supabase === 'undefined' || !global.supabase.createClient) {
      throw new Error('Supabase JS library not loaded');
    ...
    const temp = global.supabase.createClient(cfg.url, cfg.anonKey, {
      auth: {
  ...
  function onAuthStateChange(fn) {
    ensureClient().then(function (sb) {
      sb.auth.onAuthStateChange(function (event, session) {
        try { fn(event, session); } catch (e) { console.warn(e); }
      });
    }).catch(function () {});
  }
