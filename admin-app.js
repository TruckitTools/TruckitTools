...
function renderUsers() {
  ...
  el.innerHTML = list.map(function (u) {
    ...
    return '<div class="user-row"><h4>' + escapeHtml(u.email || u.username || '—') +
      ' <span style="font-size:11px;color:#94a3b8;">(' + escapeHtml(u.role || 'broker') + ')</span></h4>' +
...
async function saveUserForm() {
  ...
  if (editId) {
    ...
    if (!u) return;
    Object.assign(u, { firstName: firstName, lastName: lastName, dob: dob, phone: phone, phone2: phone2, address: address, email: email, updatedAt: new Date().toISOString() });
    saveProfiles(list);
    showApp();
    return;
  }
