...
function clearForm() {
  ...
  ['firstName','lastName','dob','phone','phone2','address','username','password','adminReauthPass'].forEach(function (id) {
...
async function saveUserForm() {
  ...
  const adminReauth = (document.getElementById('adminReauthPass') && document.getElementById('adminReauthPass').value) || '';
  if (!adminReauth) {
    err.textContent = 'Enter your admin password (below the new user password) so you stay signed in after creating the user.';
    err.classList.remove('hidden');
    return;
  }
  if (brokerCount() >= MAX_BROKERS) { err.textContent = 'Broker seat limit reached.'; err.classList.remove('hidden'); return; }

  // Remember admin credentials to re-login after signUp steals the session
  const adminEmail = (await TruckitCloud.getUser());
  const adminEmailStr = adminEmail && adminEmail.email;
  // We cannot re-auth without password — warn user they must sign in again as admin

  try {
    const data = await TruckitCloud.signUp(email, password, {
      role: 'broker', firstName: firstName, lastName: lastName
    });
    const user = data.user;
    if (!user) {
      err.textContent = 'User may need to confirm email in Supabase before login works.';
      err.classList.remove('hidden');
      // still store profile intent
    }
    upsertProfile({
    ...
    // signUp switches session to the new user — sign admin back in
    const reauth = (document.getElementById('adminReauthPass') && document.getElementById('adminReauthPass').value) || '';
    if (adminEmailStr && reauth) {
      try {
        await TruckitCloud.signIn(adminEmailStr, reauth);
        setAdminUiSession(true);
        if (document.getElementById('adminReauthPass')) document.getElementById('adminReauthPass').value = '';
        document.getElementById('userFormCard').classList.add('hidden');
        document.getElementById('adminLoginCard').classList.add('hidden');
        document.getElementById('adminApp').classList.remove('hidden');
        showApp();
        alert('Broker user created. You are still signed in as admin.');
        return;
      } catch (reErr) {
        console.warn(reErr);
        setAdminUiSession(false);
        document.getElementById('userFormCard').classList.add('hidden');
        document.getElementById('adminLoginCard').classList.remove('hidden');
        document.getElementById('adminApp').classList.add('hidden');
        if (document.getElementById('adminUser')) document.getElementById('adminUser').value = adminEmailStr || '';
        if (document.getElementById('adminPass')) document.getElementById('adminPass').value = '';
        alert('User was created, but admin re-sign-in failed. Sign in again as admin.\n\n' + ((reErr && reErr.message) || ''));
        return;
      }
    }

    // No admin password provided — user created, must sign in again
    setAdminUiSession(false);
    document.getElementById('userFormCard').classList.add('hidden');
    document.getElementById('adminLoginCard').classList.remove('hidden');
    document.getElementById('adminApp').classList.add('hidden');
    if (document.getElementById('adminUser')) document.getElementById('adminUser').value = adminEmailStr || '';
    if (document.getElementById('adminPass')) document.getElementById('adminPass').value = '';
    alert('Broker user created. Enter your admin password in the form next time (field: “Your admin password”) to stay signed in. Sign in as admin to continue.');
  } catch (e) {
