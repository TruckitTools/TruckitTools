...
function showTab(name) {
  ['users','offices','company','backup','security','cloud','purge'].forEach(function (t) {
...
function ensureOfficeFieldsOnForm() {
  if (document.getElementById('userOffice')) return;
  ...
  wrap.innerHTML =
    ...
    '<span class="field-label">Office role</span>' +
    '<select id="userOfficeRole">' +
    '<option value="owner">Owner (main broker)</option>' +
    '<option value="employee">Employee</option>' +
    '</select>';
  const title = document.getElementById('formTitle');
  if (title && title.nextSibling) form.insertBefore(wrap, title.nextSibling);
...
async function saveUserForm() {
  ...
  try {
    ...
    alert('Broker user created in Supabase Auth.\n\nImportant: Creating a user signs you out of admin on this device. Sign in again as admin to continue.\n\nIf email confirmation is ON in Supabase, the user must confirm before they can log in.');
    document.getElementById('userFormCard').classList.add('hidden');
    document.getElementById('adminLoginCard').classList.remove('hidden');
    document.getElementById('adminApp').classList.add('hidden');
    document.getElementById('adminUser').value = adminEmailStr || '';
...
(async function boot() {
  ...
  try {
    ...
    // Bootstrap button
    if (!document.getElementById('bootstrapAdminBtn')) {
      const btn = document.createElement('button');
      btn.type = 'button';
      btn.id = 'bootstrapAdminBtn';
      btn.className = 'secondary';
      btn.textContent = 'Create first admin';
      btn.style.background = '#1e40af';
      btn.style.color = '#fff';
      btn.onclick = bootstrapAdmin;
      document.getElementById('adminLoginCard').appendChild(btn);
      const hint = document.createElement('p');
      hint.className = 'muted';
      hint.style.marginTop = '10px';
      hint.textContent = 'First time: enter email + password, then Create first admin. Later: Sign In only. Old Admin/Truckit757 login is retired.';
      document.getElementById('adminLoginCard').appendChild(hint);
    }
  } catch (e) {}
