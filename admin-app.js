...
function saveOffices(list) {
  const stamped = (list || []).map(function (o) {
    return Object.assign({}, o, { updatedAt: o.updatedAt || new Date().toISOString() });
  });
  localStorage.setItem(OFFICES_KEY, JSON.stringify(stamped));
...
function renderOffices() {
  ...
  el.innerHTML = list.map(function (o) {
    ...
    const cityLine = [o.city, o.state, o.zip].filter(Boolean).join(' ').replace(/\s+/g, ' ').trim();
    const addrLine = [o.address, cityLine].filter(Boolean).join(', ');
    const ids = [o.mc ? 'MC ' + o.mc : '', o.dot ? 'DOT ' + o.dot : ''].filter(Boolean).join(' · ');
    return '<div class="user-row"><h4>' + escapeHtml(o.name) + '</h4>' +
      (contact ? '<p><strong>Contact:</strong> ' + escapeHtml(contact) + '</p>' : '') +
      (o.email ? '<p><strong>Email:</strong> ' + escapeHtml(o.email) + '</p>' : '') +
      '<p>' + escapeHtml([o.phone, o.fax ? 'Fax ' + o.fax : ''].filter(Boolean).join(' · ')) +
        (addrLine ? (o.phone || o.fax ? ' · ' : '') + escapeHtml(addrLine) : '') + '</p>' +
      (ids ? '<p>' + escapeHtml(ids) + '</p>' : '') +
      (o.website ? '<p>' + escapeHtml(o.website) + '</p>' : '') +
      (o.notes ? '<p style="color:#94a3b8;">' + escapeHtml(o.notes) + '</p>' : '') +
...
function officeFieldIds() {
  return ['officeName','officeFirstName','officeLastName','officeEmail','officePhone','officeFax',
    'officeAddress','officeCity','officeState','officeZip','officeWebsite','officeNotes'];
}

function clearOfficeForm() {
  document.getElementById('officeEditId').value = '';
  officeFieldIds().forEach(function (id) {
...
function showOfficeCreate() {
  clearOfficeForm();
  document.getElementById('officeFormTitle').textContent = 'New office';
  document.getElementById('officeAssignPanel').classList.add('hidden');
  document.getElementById('officeFormCard').classList.remove('hidden');
  document.getElementById('officeName').focus();
  try { document.getElementById('officeFormCard').scrollIntoView({ behavior: 'smooth', block: 'nearest' }); } catch (e) {}
}

function cancelOfficeForm() {
  clearOfficeForm();
  document.getElementById('officeFormCard').classList.add('hidden');
  document.getElementById('officeAssignPanel').classList.add('hidden');
}

function showOfficeEdit(id) {
  const o = getOffices().find(function (x) { return x.id === id; });
  if (!o) return;
  clearOfficeForm();
  document.getElementById('officeEditId').value = o.id;
  document.getElementById('officeFormTitle').textContent = 'Edit office';
  document.getElementById('officeName').value = o.name || '';
  document.getElementById('officeFirstName').value = o.firstName || '';
  document.getElementById('officeLastName').value = o.lastName || '';
  document.getElementById('officeEmail').value = o.email || '';
  document.getElementById('officePhone').value = o.phone || '';
  document.getElementById('officeFax').value = o.fax || '';
  document.getElementById('officeAddress').value = o.address || '';
  document.getElementById('officeCity').value = o.city || '';
  document.getElementById('officeState').value = o.state || '';
  document.getElementById('officeZip').value = o.zip || '';
  document.getElementById('officeWebsite').value = o.website || '';
  document.getElementById('officeNotes').value = o.notes || '';
  document.getElementById('officeAssignPanel').classList.remove('hidden');
  fillOfficeAssignSelect(o.id);
  document.getElementById('officeFormCard').classList.remove('hidden');
  try { document.getElementById('officeFormCard').scrollIntoView({ behavior: 'smooth', block: 'nearest' }); } catch (e) {}
}

function fillOfficeAssignSelect(officeId) {
  const sel = document.getElementById('officeAssignUser');
  if (!sel) return;
  const profiles = getProfiles().filter(function (p) {
    return p.role === 'broker' || p.role === 'admin';
  });
  sel.innerHTML = '<option value="">— Select user —</option>' + profiles.map(function (p) {
    const label = [p.firstName, p.lastName].filter(Boolean).join(' ') || p.email || p.id;
    const tag = p.officeId === officeId ? ' (already here)' : (p.officeId ? ' (other office)' : '');
    return '<option value="' + p.id + '">' + escapeHtml(label + ' · ' + (p.email || '') + tag) + '</option>';
  }).join('');
}

function assignUserToOffice() {
  const officeId = document.getElementById('officeEditId').value;
  if (!officeId) { alert('Save the office first, then assign people.'); return; }
  const userId = document.getElementById('officeAssignUser').value;
  const role = document.getElementById('officeAssignRole').value || 'employee';
  if (!userId) { alert('Select a user.'); return; }
  const list = getProfiles();
  const u = list.find(function (p) { return p.id === userId; });
  if (!u) return;
  // Only one owner per office
  if (role === 'owner') {
    list.forEach(function (p) {
      if (p.officeId === officeId && p.officeRole === 'owner' && p.id !== userId) {
        p.officeRole = 'employee';
      }
    });
  }
  u.officeId = officeId;
  ...
  saveProfiles(list);
  fillOfficeAssignSelect(officeId);
  renderOffices();
  renderUsers();
  const ok = document.getElementById('officeOk');
  ok.textContent = 'Team member updated.';
  ok.classList.remove('hidden');
}

function unassignFromOffice(userId) {
  if (!confirm('Remove this person from the office?')) return;
  const list = getProfiles();
  const u = list.find(function (p) { return p.id === userId; });
  if (!u) return;
  u.officeId = '';
  ...
  saveProfiles(list);
  const officeId = document.getElementById('officeEditId').value;
  if (officeId) fillOfficeAssignSelect(officeId);
  renderOffices();
  renderUsers();
}
...
function saveOfficeForm() {
  ...
  const name = v('officeName');
  if (!name) { err.textContent = 'Office name is required.'; err.classList.remove('hidden'); return; }
  const editId = document.getElementById('officeEditId').value;
  const list = getOffices();
  const payload = {
    name: name,
    firstName: v('officeFirstName'),
    lastName: v('officeLastName'),
    email: v('officeEmail'),
    phone: v('officePhone'),
    fax: v('officeFax'),
    ...
    state: v('officeState'),
    zip: v('officeZip'),
    website: v('officeWebsite'),
    notes: v('officeNotes'),
    updatedAt: new Date().toISOString()
  };
  if (editId) {
    const idx = list.findIndex(function (o) { return o.id === editId; });
    if (idx < 0) { err.textContent = 'Office not found.'; err.classList.remove('hidden'); return; }
    list[idx] = Object.assign({}, list[idx], payload);
  } else {
    payload.id = 'off_' + Date.now().toString(36);
    payload.createdAt = new Date().toISOString();
    list.push(payload);
    document.getElementById('officeEditId').value = payload.id;
    document.getElementById('officeFormTitle').textContent = 'Edit office';
    document.getElementById('officeAssignPanel').classList.remove('hidden');
    fillOfficeAssignSelect(payload.id);
  }
  saveOffices(list);
...
function renderOffices() {
  ...
  const profiles = getProfiles();
  if (!list.length) {
    el.innerHTML = '<p class="muted">No offices yet. Tap <strong>+ New office</strong> to add one.</p>';
    return;
  }
  el.innerHTML = list.map(function (o) {
    const members = profiles.filter(function (p) { return p.officeId === o.id; });
    const owner = members.find(function (m) { return m.officeRole === 'owner'; });
    const emps = members.filter(function (m) { return m.officeRole !== 'owner'; });
    const contact = [o.firstName, o.lastName].filter(Boolean).join(' ');
    const cityLine = [o.city, o.state, o.zip].filter(Boolean).join(' ').replace(/\s+/g, ' ').trim();
    const addrLine = [o.address, cityLine].filter(Boolean).join(', ');
    let membersHtml = '<div class="office-members"><h5>Team (' + members.length + ')</h5>';
    if (!members.length) {
      membersHtml += '<p class="muted" style="margin:0;font-size:12px;">No one assigned yet. Edit office to add people, or set office on a user.</p>';
    } else {
      const ordered = (owner ? [owner] : []).concat(emps);
      membersHtml += ordered.map(function (m) {
        const label = [m.firstName, m.lastName].filter(Boolean).join(' ') || m.email || m.id;
        const role = m.officeRole === 'owner' ? 'Owner' : 'Employee';
        return '<div class="member-chip"><span><span class="role-tag">' + role + '</span> ' +
          escapeHtml(label) + (m.email ? ' <span style="color:#64748b;">' + escapeHtml(m.email) + '</span>' : '') +
          '</span><button type="button" class="ghost" style="padding:4px 8px;font-size:12px;" onclick="event.stopPropagation();unassignFromOffice(\'' + m.id + '\')">Remove</button></div>';
      }).join('');
    }
    membersHtml += '</div>';
    return '<div class="office-card">' +
      '<div class="office-card-header"><div>' +
      '<h4>' + escapeHtml(o.name) + '</h4>' +
      (contact ? '<div class="office-meta">' + escapeHtml(contact) + '</div>' : '') +
      (o.email ? '<div class="office-meta">' + escapeHtml(o.email) + '</div>' : '') +
      '<div class="office-meta">' + escapeHtml([o.phone, o.fax ? ('Fax ' + o.fax) : ''].filter(Boolean).join(' · ')) + '</div>' +
      (addrLine ? '<div class="office-meta">' + escapeHtml(addrLine) + '</div>' : '') +
      (o.website ? '<div class="office-meta">' + escapeHtml(o.website) + '</div>' : '') +
      (o.notes ? '<div class="office-meta" style="font-style:italic;">' + escapeHtml(o.notes) + '</div>' : '') +
      '</div><div class="office-actions">' +
      '<button type="button" class="secondary" onclick="showOfficeEdit(\'' + o.id + '\')">Edit</button>' +
      '<button type="button" class="cancel" onclick="deleteOffice(\'' + o.id + '\')">Delete</button>' +
      '</div></div>' + membersHtml + '</div>';
  }).join('');
}

function saveNewOffice() { showOfficeCreate(); }

function deleteOffice(id) {
  if (!confirm('Delete this office? People stay in the system but are unassigned from it.')) return;
  ...
  if (document.getElementById('officeEditId').value === id) cancelOfficeForm();
  renderOffices();
  renderUsers();
  fillOfficeSelect();
}
...
const DEFAULT_COMPANY = {
  name: 'Transent LLC', address: '23 Winnard Rd.', city: 'Hampton', state: 'VA', zip: '23669',
...
function hideAdminBootStatus() {
  const boot = document.getElementById('adminBootStatus');
...
function showTab(name) {
  const tabs = ['offices','users','company','backup','security','cloud','integrations','purge'];
  ...
  if (name === 'integrations') loadAdminFdwKey();
  if (name === 'offices') {
...
function loadAdminFdwKey() {
  const el = document.getElementById('adminFdwApiKey');
  const ok = document.getElementById('fdwKeyOk');
  const err = document.getElementById('fdwKeyErr');
  if (ok) ok.classList.add('hidden');
  if (err) err.classList.add('hidden');
  if (!el) return;
  try {
    el.value = localStorage.getItem(FDW_KEY_STORAGE) || '';
  } catch (e) {
    el.value = '';
  }
}

function saveAdminFdwKey() {
  const el = document.getElementById('adminFdwApiKey');
  const ok = document.getElementById('fdwKeyOk');
  const err = document.getElementById('fdwKeyErr');
  if (ok) ok.classList.add('hidden');
  ...
  if (!v) {
    if (err) { err.textContent = 'Enter an API key first.'; err.classList.remove('hidden'); }
    return;
  }
  try {
    localStorage.setItem(FDW_KEY_STORAGE, v);
    if (ok) { ok.textContent = 'FDW API key saved. TruckitRates and Get Rate will use it.'; ok.classList.remove('hidden'); }
  } catch (e) {
    if (err) { err.textContent = 'Could not save key locally.'; err.classList.remove('hidden'); }
  }
}

function clearAdminFdwKey() {
  const el = document.getElementById('adminFdwApiKey');
  const ok = document.getElementById('fdwKeyOk');
  const err = document.getElementById('fdwKeyErr');
  if (ok) ok.classList.add('hidden');
  if (err) err.classList.add('hidden');
  try { localStorage.removeItem(FDW_KEY_STORAGE); } catch (e) {}
  if (el) el.value = '';
  if (ok) { ok.textContent = 'FDW API key cleared.'; ok.classList.remove('hidden'); }
}
...
function primaryAdminEmail() {
  const admins = getProfiles().filter(function (p) { return p.role === 'admin'; });
  if (admins.length && admins[0].email) return String(admins[0].email);
  try {
    const sess = window.TruckitBrokerAuth && TruckitBrokerAuth.getBrokerSession && TruckitBrokerAuth.getBrokerSession();
    if (sess && sess.role === 'admin' && sess.email) return String(sess.email);
  } catch (e) {}
  return '';
}
...
(async function boot() {
  ...
  try {
    ...
    if (u) {
      u.type = 'email';
      u.placeholder = 'Admin account email';
      u.value = primaryAdminEmail();
    }
  } catch (e) {}
