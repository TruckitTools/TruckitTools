...
(function (global) {
  ...
  function injectAdminButton() {
    // Admin Tools lives only on home (index.html) as a static header link.
    // On every other page, remove any leftover injected button.
    const existing = document.getElementById('ttAdminToolsBtn');
    if (!isHomeLanding()) {
      if (existing && existing.tagName === 'BUTTON') existing.remove();
      return;
    }
    // Home: prefer static <a class="admin-tools-btn"> from index.html — do not inject a second button
    if (existing) return;
